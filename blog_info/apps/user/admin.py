...
class CustomUserAdmin(UserAdmin): 
    ...
    def is_registered(self, obj): 
        return obj.groups.filter(name='Registereds').exists() 
    ...
    def is_admin(self, obj): 
        return obj.groups.filter(name='Administrators').exists() 
    ...
    def add_to_registered(self, request, queryset):
        ...
        self.message_user(
             request, "Los usuarios seleccionados fueron añadidos al grupo 'Registereds'.") 
    ...
    def add_to_admins(self, request, queryset): 
        admins_group = Group.objects.get(name='Administrators') 
        for user in queryset: 
            user.groups.add(admins_group) 
        self.message_user( 
            request, "Los usuarios seleccionados fueron añadidos al grupo  'Administrators'.") 
    ...
    def remove_from_registered(self, request, queryset):
        ...
        self.message_user( 
            request, "Los usuarios seleccionados fueron removidos del grupo 'Registereds'.") 
    ...
    def remove_from_admins(self, request, queryset): 
        ...
        self.message_user( 
            request, "Los usuarios seleccionados fueron removidos del grupo 'Administrators'.") 
